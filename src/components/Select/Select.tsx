...
import { ChildrenMap, groupChildrenAsDataSource, Key, SubGroupMap } from '../hooks/group';
import { getPopoverConfigFromContext } from '../hooks/usePopoverConfig';
...
const warnLogForVirtualList = once(() =>
  console.error('Select virtualList only valid when use options'),
);
...
> & {
  ...
  return (
    ...
    >
      ...
      <Arrow key="icon" type={visible ? 'angle_up' : 'angle_down'} />
    </SSelector>
  );
};
  ...
  const group = (options: T[], disabled = false, prefixKey: Key): [Key[], Key[], ReactNode[]] => {
    ...
    options.forEach((child, i) => {
      ...
      if (subChildren) {
        ...
        const reactKey =
          child.key || isValidKey(child[subGroupKeyName])
            ? child[subGroupKeyName]
            : `${prefixKey}-${i}`;
        ...
        const reactKey =
          child.key || isValidKey(child[itemKeyName]) ? child[itemKeyName] : `${prefixKey}-${i}`;
        const isDisabled = disabled || child.disabled;
        if (isDisabled) {
...
const Select = ({
  ...
  const optionsDataSource = useMemo(
    () =>
      options
        ? groupOptionsAsDataSource(options, disabled, groupOptions, searchValue, handleSearch)
        : [],
