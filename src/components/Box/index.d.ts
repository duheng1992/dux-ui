...
declare class Box extends PureComponent<BoxProps> {
  static propTypes: {
     ...
     * @description 子组件间的间距，预设 sm md lg 等内置尺寸，或自定义间距大小(传入 number，单位为 px)，间距方向跟随布局方向
     */
    spacing: PropTypes.Requireable<string | number | (string | number | null | undefined)[]>;
    /**
     * @description 布局方向，默认为 row 横向布局，column 为纵向布局，加上 -reverse 为单行（单列）子元素顺序反转
     */
    ...
    /** 间距，可以为 sm md lg 等内置尺寸，或者为自定义字符串，或传入数组 [横向 padding, 纵向 padding]，或有效的 padding 字符串 */
    padding: PropTypes.Requireable<string | number | (string | number | null | undefined)[]>;
    /** 宽度, 同 css 宽度 */
    ...
    cleanMargin: PropTypes.Requireable<boolean>;
  };
